...
exports.loadBlogs = (path) => {
  ...
  debug('>>> bogFolder', blogFolder);
  return blogFolder;
};

const loadYamlInfo = (path) => {
  // const content = fs.readFileSync(path);
  const source = yamlFront.loadFront(fs.readFileSync(path));
  const content = marked(source.__content);
...
exports.generateIndex = (info, blogPath) =>
  info.map((item) => ({
    ...
    meta: loadYamlInfo(path.join(blogPath, item.dir, item.base))
  }));
